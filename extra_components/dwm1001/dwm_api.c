...
void dwm_init(uint8_t * spidev)
{
   LMH_Init(spidev);
}

void dwm_deinit(uint8_t * spidev)
{
   LMH_DeInit(spidev);
}

int dwm_pos_set(uint8_t * spidev, dwm_pos_t* pos)
{
   ...
   *(uint32_t*)(tx_data+tx_len) = pos->z;
   ...
   LMH_Tx(spidev,tx_data, &tx_len);
   return LMH_WaitForRx(spidev, rx_data, &rx_len, 3);
}

int dwm_pos_get(uint8_t * spidev, dwm_pos_t* p_pos)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 18) == RV_OK)
   {
...
int dwm_upd_rate_set(uint8_t * spidev, uint16_t ur, uint16_t ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_upd_rate_get(uint8_t * spidev, uint16_t *ur, uint16_t *ur_static)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 9) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_cfg_tag_set(uint8_t * spidev, dwm_cfg_tag_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_cfg_anchor_set(uint8_t * spidev, dwm_cfg_anchor_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_cfg_get(uint8_t * spidev, dwm_cfg_t* cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 7) == RV_OK)
   {
...
int dwm_anchor_list_get(uint8_t * spidev, dwm_anchor_list_t *p_list)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, DWM1001_TLV_MAX_SIZE) == RV_OK)
   {
...
int dwm_loc_get(uint8_t * spidev, dwm_loc_data_t* loc)
{ 
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_baddr_set(uint8_t * spidev, dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_baddr_get(uint8_t * spidev, dwm_baddr_t* p_baddr)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 11) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_stnry_cfg_set(uint8_t * spidev, dwm_stnry_sensitivity_t sensitivity)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_stnry_cfg_get(uint8_t * spidev, dwm_stnry_sensitivity_t* p_sensitivity)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 6) == RV_OK)
   {
...
int dwm_factory_reset(uint8_t * spidev)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_ver_get(uint8_t * spidev, dwm_ver_t* ver)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 21) == RV_OK)
   {    
      // read fw_version
...
int dwm_uwb_cfg_set(uint8_t * spidev, dwm_uwb_cfg_t* p_cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_uwb_cfg_get(uint8_t * spidev, dwm_uwb_cfg_t* p_cfg)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0;   
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 15) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_usr_data_read(uint8_t * spidev, uint8_t* p_data, uint8_t* p_len)
{     
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_usr_data_write(uint8_t * spidev, uint8_t* p_data, uint8_t len, bool overwrite)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_label_read(uint8_t * spidev, uint8_t* p_label, uint8_t* p_len)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_label_write(uint8_t * spidev, uint8_t* p_label, uint8_t len)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_gpio_cfg_output(uint8_t * spidev, dwm_gpio_idx_t idx, bool value)
{      
...
int dwm_gpio_cfg_input(uint8_t * spidev, dwm_gpio_idx_t idx, dwm_gpio_pin_pull_t pull_mode)
{      
   if(LMH_CheckGPIOIdx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_set(uint8_t * spidev, dwm_gpio_idx_t idx, bool value)
{
   if(LMH_CheckGPIOIdx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_get(uint8_t * spidev, dwm_gpio_idx_t idx, bool* p_value)
{
   if(LMH_CheckGPIOIdx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_gpio_value_toggle(uint8_t * spidev, dwm_gpio_idx_t idx)
{
   if(LMH_CheckGPIOIdx(idx)!= RV_OK)
   {
      return RV_ERR;
...
int dwm_panid_set(uint8_t * spidev, uint16_t value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_panid_get(uint8_t * spidev, uint16_t* p_value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_node_id_get(uint8_t * spidev, uint64_t *p_node_id)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
   ...
   tx_data[tx_len++] = 0; 
   LMH_Tx(spidev,tx_data, &tx_len);
   if(LMH_WaitForRx(spidev, rx_data, &rx_len, 13) == RV_OK)
   {
      data_cnt = RESP_DAT_VALUE_OFFSET;
...
int dwm_status_get(uint8_t * spidev, dwm_status_t* p_status)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_int_cfg_set(uint8_t * spidev, uint16_t value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_int_cfg_get(uint8_t * spidev, uint16_t *p_value)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_bh_status_get(uint8_t * spidev, bh_status_t * p_bh_status)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
extern uint8_t nodes[2][64];

int dwm_backhaul_xfer(uint8_t * spidev)
{
   uint8_t tx_data[2*DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[5*DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len = 0;
//   uint8_t data_cnt, i;
   tx_data[tx_len++] = DWM1001_TLV_TYPE_CMD_BACKHAUL_XFER;
   tx_data[tx_len++] = 0x02;
   tx_data[tx_len++] = 0x01;
   tx_data[tx_len++] = 0x00;

   LMH_Tx(spidev,tx_data, &tx_len);
   //return LMH_SPIRX_XFER_DRDY_WaitForRx(rx_data, &rx_len, 5*DWM1001_TLV_MAX_SIZE);

   if(LMH_SPIRX_XFER_DRDY_WaitForRx(spidev, rx_data, &rx_len, 5*DWM1001_TLV_MAX_SIZE) == RV_OK)
   {
	   //printf("###############rx_len: %d\n",rx_len);
	   //if(rx_len>0) printf("*****rx_data[0]: %d\n",rx_data[0]);
	   //uint8_t j;
	   //for(j=0;j<rx_len;j++) printf("data[%d]:0x%02x\n",j,rx_data[j]);
	   //printf("\n");
//	   if(rx_len==24) for(j=0;j<rx_len;j++) printf("data[%d]:0x%02x\n",j,rx_data[j]);
	   //printf("node id:0x%02x%02x, len:%d \n",rx_data[9],rx_data[8],rx_len);

	   //if(rx_len > 10)printf("node id:0x%02x%02x, len:%d \n",rx_data[9],rx_data[8],rx_len);
	   //if(rx_len >10 && rx_data[9]==0x5b && rx_data[8]==0x0d){

//	   bool node_exists=0;
//	   if(rx_len>10){
//		   printf("node id:0x%02x%02x, len:%d \n",rx_data[9],rx_data[8],rx_len);
//		   for(int i=0; i<64; i++) {
//			   if(rx_data[9]==nodes[0][i] && rx_data[8]==nodes[1][i]) {
//				   printf("node exists\n");
//				   node_exists=1;
////				   if(rx_len==68 && rx_data[33]==0x0f){
////					   printf("node id:0x%02x%02x, len:%d \n",rx_data[9],rx_data[8],rx_len);
////					   printf("SuperFrame Number: %d\n",rx_data[1]<<8|rx_data[0]);
////					   printf("x: %d mm\n",rx_data[36]<<24 | rx_data[37]<<16 | rx_data[38]<<8 | rx_data[39]);
////					   printf("y: %d mm\n",rx_data[40]<<24 | rx_data[41]<<16 | rx_data[42]<<8 | rx_data[43]);
////					   printf("z: %d mm\n",rx_data[44]<<24 | rx_data[45]<<16 | rx_data[46]<<8 | rx_data[47]);
////					   printf("\n");
////				   }
//
//				   break;
...
//		   }
//		   if(node_exists==0){
//			   printf("node NOT exists..sending tx\n");
//			   for(int j=0; j<64; j++) {
//				   if( nodes[0][j]==0x00 && nodes[1][j]==0x00) {
//					   printf("writing node\n");
//					   nodes[0][j]= rx_data[9];
//					   nodes[1][j]=rx_data[8];
//					   break;
...
//			   tx_data[tx_len++] = DWM1001_TLV_TYPE_CMD_BACKHAUL_XFER;
//				tx_data[tx_len++] = 0x02;
//				if(rx_len==48) tx_data[tx_len++] = 0x32;
//				else tx_data[tx_len++] = 0x5a;
//				tx_data[tx_len++] = 0x00;
//				LMH_Tx(spidev,tx_data, &tx_len);
//				LMH_SPIRX_XFER_DRDY_WaitForRx(rx_data, &rx_len, 5*DWM1001_TLV_MAX_SIZE);
//		   }
//
//	   }

	   //demonstração:
	   if(rx_len==68 && rx_data[9]==0x0c && rx_data[8]==0xde && rx_data[33]==0x0f){
	   	   printf("node id:0x%02x%02x, len:%d \n",rx_data[9],rx_data[8],rx_len);
	   	   printf("SuperFrame Number: %d\n",rx_data[1]<<8|rx_data[0]);
		   printf("x: %d mm\n",rx_data[36]<<24 | rx_data[37]<<16 | rx_data[38]<<8 | rx_data[39]);
		   printf("y: %d mm\n",rx_data[40]<<24 | rx_data[41]<<16 | rx_data[42]<<8 | rx_data[43]);
...
//	   if(rx_len>4 && rx_len!=68 && rx_len!=48){
//		   for(j=0;j<rx_len;j++) printf("data[%d]:0x%02x\n",j,rx_data[j]);
//	   }
...
int dwm_enc_key_set(uint8_t * spidev, dwm_enc_key_t* p_key)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint8_t i;
...
int dwm_enc_key_clear(uint8_t * spidev )
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_uwb_preamble_code_set(uint8_t * spidev, dwm_uwb_preamble_code_t code)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_uwb_preamble_code_get(uint8_t * spidev, dwm_uwb_preamble_code_t *p_code)
{
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_uwb_scan_start(uint8_t * spidev)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint16_t rx_len;
...
int dwm_uwb_scan_result_get(uint8_t * spidev, dwm_uwb_scan_result_t *p_result)
{        
   uint8_t tx_data[DWM1001_TLV_MAX_SIZE], tx_len = 0;
   uint8_t rx_data[DWM1001_TLV_MAX_SIZE];
   uint8_t data_cnt, i;
